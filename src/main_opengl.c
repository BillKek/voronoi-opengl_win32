...
void render_video_mode(GLFWwindow *window)
{
    ...
    unsigned long frames_count = floorf(duration/delta_time);

    for (unsigned long i = 0; i < frames_count && !glfwWindowShouldClose(window); ++i) {
        ...
        snprintf(file_path, sizeof(file_path), "%s/frame-%03lu.png", output_dir, i);
        ...
        printf("INFO: Rendered %lu/%lu frames\n", i + 1, frames_count);
